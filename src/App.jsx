...
var TRANSLATE_SYSTEM = "Tu es un traducteur rap. On te donne les PAROLES EXACTES d'un morceau. Ton travail: traduire ligne par ligne et decrypter.\n\nREGLE: reproduis CHAQUE ligne originale telle quelle dans le champ \"o\", et mets la traduction francaise dans \"t\". Ne modifie JAMAIS les paroles originales. Ne saute AUCUNE ligne. Garde l'ordre exact.\n\nReponds en JSON:\n{\n\"lang\":\"anglais\",\n\"lines\":[\n{\"s\":\"[Intro]\"},\n{\"o\":\"ligne originale exacte\",\"t\":\"traduction francaise\"},\n{\"s\":\"[Couplet 1]\"},\n{\"o\":\"...\",\"t\":\"...\"},\n{\"s\":\"[Refrain]\"},\n{\"o\":\"...\",\"t\":\"...\"}\n],\n\"notes\":[\n{\"r\":\"mot/expression\",\"e\":\"explication concise: slang, ref culturelle, wordplay, double sens\"}\n]\n}\n\n\"s\"=section header (garde les headers des paroles originales). \"o\"=ligne originale EXACTE copiee mot pour mot. \"t\"=traduction francaise.\nTraduis le SENS pas mot a mot. Si francais: \"t\"=null.\nDecryptage uniquement sur les passages opaques.\nNe traduis PAS les noms propres, les ad-libs ou les tags producteur.";
...
export default function App() {
  ...
  var decode = useCallback(async function(name, autoMode) {
    ...
    if (!autoMode) setSel(name);
    try {
      // Step 1: Get exact lyrics from Genius
      var genius = await fetchLyrics(name, artist);

      if (!genius.found) {
        up({ st: "ok", d: { found: false, lang: "?", lines: [], notes: [], _source: null } });
        setDone(function(p) { return p + 1; });
        return;
      }

      // Step 2: Send exact lyrics to Gemini for translation only (no web search)
      var prompt = "Voici les paroles EXACTES de \"" + name + "\" par " + artist + " (source: Genius).\nCopie chaque ligne originale mot pour mot dans le champ \"o\". Ne modifie rien.\n\nPAROLES:\n\n" + genius.lyrics;
                ...
                <div style={{ animation: "fadeIn .2s ease" }}>
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 5, flexWrap: "wrap" }}>
                      ...
                      {curD.found
                        ? <span style={Object.assign({}, S.tag, { color: "#4ade80" })}>genius.com</span>
                        : <span style={Object.assign({}, S.tag, { color: "#f0c040" })}>pas sur Genius</span>}
                  ...
                  {!curD.found && (
                    <div style={{ color: "#444", fontSize: 11, padding: 16 }}>Paroles introuvables sur Genius pour ce morceau.</div>
                  )}
                </div>
              )}
            </div>
