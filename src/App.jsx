...
var TRANSLATE_SYSTEM = "Tu es un traducteur rap. On te donne les PAROLES EXACTES d'un morceau.\n\nREGLES:\n1. Regroupe les lignes trop courtes qui font partie de la meme phrase en UNE seule ligne.\n2. Garde les sections [Intro], [Verse], [Chorus] etc.\n3. Champ \"o\" = ligne originale (regroupee si necessaire). \"t\" = traduction francaise.\n4. Pour CHAQUE ligne, ajoute \"c\" = ton niveau de confiance (0-100). 100 = traduction evidente. <70 = slang rare, ref obscure, wordplay difficile, sens incertain.\n5. Si francais: \"t\"=null.\n6. CONTEXTE RAP: \"bitch\" = meuf/copine (JAMAIS pute). \"nigga\" = ne traduis pas, laisse tel quel ou dis \"negro\". \"whip\" = caisse. Adapte au registre du rap francais, pas du francais scolaire.\n\nPour les notes de decryptage, donne pour chaque ref:\n- \"r\" = mot/expression dans le morceau\n- \"e\" = explication concise (1-2 phrases)\n- \"t\" = type: \"slang\" / \"ref\" (ref culturelle) / \"wordplay\" / \"sample\" (ref a un autre morceau)\n\nReponds en JSON:\n{\n\"lang\":\"anglais\",\n\"lines\":[\n{\"s\":\"[Intro]\"},\n{\"o\":\"ligne originale\",\"t\":\"traduction francaise\",\"c\":95},\n{\"s\":\"[Verse 1]\"},\n{\"o\":\"...\",\"t\":\"...\",\"c\":60,\"lineId\":\"v1-l1\"}\n],\n\"notes\":[\n{\"r\":\"mot/expression\",\"e\":\"explication\",\"t\":\"ref\"}\n]\n}";

var DEEP_ANALYSIS_SYSTEM = "Tu es un analyste rap expert. On te donne UNE ligne d'un morceau, le contexte du morceau (artiste, album, section), et les lignes qui entourent.\n\nAnalyse cette ligne en profondeur:\n- Sens litteral\n- Sens figure / ce que l'artiste veut vraiment dire\n- Refs (personnes, marques, lieux, evenements, morceaux samples)\n- Wordplay, double sens, homophones\n- Technique (flow, rime, schema)\n- Contexte dans le morceau/album si pertinent\n\nReponds en JSON:\n{\n\"literal\":\"sens litteral\",\n\"meaning\":\"ce que l'artiste dit vraiment\",\n\"refs\":[{\"r\":\"ref\",\"e\":\"explication\"}],\n\"wordplay\":\"wordplay si present, sinon null\",\n\"technique\":\"note sur la technique si notable, sinon null\"\n}";
...
export default function App() {
  ...
  var decode = useCallback(async function(name, autoMode) {
    ...
    if (!autoMode) setSel(name);
    try {
      var genius = await fetchLyrics(name, artist, album);
      ...
      if (genius.found) {
        ...
        var r2 = await callGemini(FALLBACK_SYSTEM, "Trouve et traduis les paroles de \"" + name + "\" par " + artist + ", album \"" + album + "\".", true);
  ...
  var analyzeLine = async function(lineIdx, line) {
    ...
    try {
      ...
      var prompt = "Morceau: \"" + sel + "\" par " + artist + " (album: " + album + ").\n\nContexte (lignes autour):\n" + contextLines.join("\n") + "\n\nLIGNE A ANALYSER: " + line.o + "\n\nTraduction actuelle: " + (line.t || line.o);
    ...
    <div style={S.root}>
      ...
      {view === "input" && (
        <div style={S.inputWrap}>
          <Inp label="Artiste" val={artist} set={setArtist} ph="Westside Gunn" enter={go} />
...
var S = {
  ...
  linePair: { marginBottom: 5, padding: "3px 6px", marginLeft: -6, marginRight: -6, borderRadius: 4, transition: "background .15s" },
